...
"""--------------------------------------------------------------------
 * $Id: surface_options.py 3155 2015-08-10 10:48:25Z robert.buras $
 * 
...
class setup_surface_group():
	...
	def __init__(self):
		...
		brdf_non_parents=['albedo','albedo_file','albedo_library',
                                  ...
                                  'mc_rossli_file', 'mc_ambrals_file', 'mc_rpv_type', 'mc_rpv_file',
                                  'bpdf_tsang_u10']
		...
		mc_albedo_type = option_definition.option(
			...
			gui_inputs=(GUI_definition.FileInput(name='Input.rte.mc.filename[FN_MC_ALBEDOTYPE]'),),
			tokens=option_definition.addToken( name='Input.rte.mc.filename[FN_MC_ALBEDOTYPE]',datatype=io.IOBase ),	
			parents=['uvspec'], 
			non_parents=brdf_non_parents,
		...
		mc_rpv_type = option_definition.option(
			name='mc_rpv_type',
			group='surface', 
			helpstr='Define a MYSTIC 2D rpv albedo type file.',
			documentation=documentation['mc_rpv_type'],
			gui_inputs=(GUI_definition.FileInput(name='Input.rte.mc.filename[FN_MC_RPVTYPE]'),),
			tokens=option_definition.addToken( name='Input.rte.mc.filename[FN_MC_RPVTYPE]',datatype=io.IOBase ),	
			parents=['uvspec'], 
			non_parents=brdf_non_parents,
                        non_parent_exceptions=['mc_rpv_file'],
			speaker='rte_solver',
			enable_values=("mystic","montecarlo"),
			threedmystic =True
		...
		mc_rpv_file = option_definition.option(
			name='mc_rpv_file',
			group='surface', 
			helpstr='Define a MYSTIC 2D rpv albedo input file.',
		...
		self.options = [altitude,
				...
				bpdf_tsang_u10,
				brdf_rpv_library, 
				brdf_rpv, brdf_rpv_type, mc_rpv_type, 
				brdf_hapke, brdf_hapke_file,
				brdf_rpv_file, mc_rpv_file, 
...
def get_surface_documentation():
	...
	mc\_rpv\_file file
		...
		'mc_rpv_type' : r'''
	File containing wavelength-dependent RPVs to be used in combination with \code{mc\_rpv\_file}.
	\fcode{
	mc\_rpv\_type file
	}
		''',
	...
	(that is as minimal configuration u10).
		...
		'mc_albedo_file' : r'''
	Define a MYSTIC 2D albedo input file (default: albedo2D.dat).
	\fcode{
