"""--------------------------------------------------------------------
 * $Id: output_options.py 3125 2015-06-09 15:52:38Z Claudia.Emde $
 * 
...
class setup_output_group():
	...
	def __init__(self):
		...
		mc_backward_output = option_definition.option(
			...
			tokens=[ option_definition.addLogical(name='Input.rte.mc.backward.output', logicals=['edir','edn','eup','fdir','fdn','fup','act','abs','emis','heat'], setting='MCBACKWARD_', gui_name='output' ),	
		...
		mc_basename = option_definition.option(
			name='mc_basename', 
			group='output', 
			helpstr='Filename for MYSTIC 3D output.',
