"""--------------------------------------------------------------------
 * $Id: solver_options.py 3524 2019-12-23 01:49:09Z bernhard.mayer $
 * 
    ...
    def __init__(self):
        ...
        rte_solver = option_definition.option(
            ...
            tokens=option_definition.addLogical(name='Input.rte.solver', 
                        logicals = [ 'disort', 'twostr', 'mystic', 'rodents', 'sslidar', 'null','sdisort', 'spsdisort', 
                        'fdisort1', 'fdisort2', 'sos', 'polradtran', 'ftwostr', 'montecarlo', 'tzs', 'sssi', 'sss', 
                        'twostrebe', 'twomaxrnd', 'twomaxrnd3C'],   
            setting='SOLVER_'),
            parents=['uvspec'],
            childs=['polradtran_quad_type','polradtran','polradtran_max_delta_tau',
                     'pseudospherical', 'disort_spherical_albedo',
                     'disort_intcor', 'isotropic_source_toa','raman',
                     ...
                     'mc_backward_output', 'mc_backward_writeallpixels', 'mc_basename',
                     'mc_delta_scaling', 'mc_elevation_file', 'mc_tenstream',
                     'mc_ipa','mc_tipa','ipa_3d','tipa',
                     'mc_maxscatters','mc_minscatters','mc_minphotons','mc_photons', 
                     'mc_photons_file','mc_polarisation',
                     'mc_wcloud_file','mc_icloud_file','mc_randomseed', 'mc_rpv_file','mc_rpv_spectral_file',
                     'mc_rpv_type','mc_ambrals_spectral_file','mc_ambrals_type','mc_spherical','mc_std',
                     ...
                     'tzs_cloud_top_height', 'twomaxrnd3C_scale_cf' , 'mc_nca'    # CK2019 added nca    
                      ],
...
#no_sdisort     )
        ...
        mc_tenstream = option_definition.option(
            ...
            tokens=[option_definition.addSetting(name='Input.rte.mc.tenstream', setting=1,default=0),
                    option_definition.addToken(name='Input.rte.mc.tenstream_options', datatype=str, optional=True, default="NULL")],
            parents=['uvspec'],
            speaker='rte_solver',
...
#        )
        mc_nca = option_definition.option(
            ...
            tokens=[option_definition.addSetting(name='Input.rte.mc.nca', setting=1,default=0), 
                    option_definition.addToken(name='Input.rte.mc.nca_options', datatype=str, optional=True, default="NULL")],
            parents=['uvspec'],
            speaker='rte_solver',
            enable_values=("mystic",),
...
def get_documentation():
    return get_solver_documentation()
...
def get_solver_documentation():
    ...
    Define cloud top height of "blackbody clouds" for radiative transfer solver \code{tzs}.
        ''',
#        'twomaxrnd3C_scale_cf' : r'''
#    Scale factor for cloud fraction split for radiative transfer solver \code{twomaxrnc3C}.
#        ''',
        'mc_nca' : r'''
